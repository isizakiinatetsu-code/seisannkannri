...
export async function GET() {
  try {
    ...
    const headers = ['納入予定日', '納入予定時刻', '物件名', '品目', '内容・規格', '業者名', '降し場所', '保管場所', '数量', '単位', '発注番号', '備考', 'ステータス', '一部納入', '納入確認時刻', '追加者'];
    const esc = (v: unknown) => {
    ...
    for (const d of rows) {
      lines.push([
        ...
        d.notes, d.status, d.is_partial ? '一部納入' : '', d.delivered_at, d.created_by,
      ].map(esc).join(','));
    }
